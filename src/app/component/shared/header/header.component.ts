import { ViewportScroller } from '@angular/common';
import { Component } from '@angular/core';
import { NavigationEnd, Router } from '@angular/router';
import { environment } from 'src/environments/environment';

interface ILangOpt {
  id: string;
  label: string;
  icon: string;
  url: string;
}

@Component({
  selector: 'app-header',
  templateUrl: './header.component.html',
  styleUrls: ['./header.component.scss'],
})
export class HeaderComponent {
  showContactUsButton = true;
  languages: ILangOpt[] = [];

  constructor(
    private viewportScroller: ViewportScroller,
    private router: Router
  ) {
    router.events.subscribe((event) => {
      if (event instanceof NavigationEnd) {
        this.showContactUsButton = !(router.url === '/legal-notice');
      }
    });
  }

  ngOnInit() {
    this.languages = [
      {
        id: 'fr',
        label: $localize`:@@language_fr:Français`,
        icon: 'data:image/png;base64,',
        url: environment.defaultURL,
      },
      {
        id: 'en',
        label: $localize`:@@language_en:English`,
        icon: 'data:image/png;base64,',
        url: environment.englishURL,
      },
    ];
  }

  scrollToContent(elementId: string) {
    this.viewportScroller.scrollToAnchor(elementId);
  }
}
